...
def insert_csv():
    if request.method == 'POST':
        ...
        for row in csv_reader:
            ...
            # Se construye diccionario de proveedores: {nombre_empresa: id}
            cur.execute("SELECT id, nombre_empresa FROM datosproveedorsalud")
            proveedores = cur.fetchall()
...
@app.route('/guardar_historial_masivo', methods=['POST'])
@login_required
...
def update_estado_equipo():
    if request.method == 'POST':
        ...
        quien_entrega_equipo = request.form.get('quien_entrega_equipo')
        ...
        fecha_prestamo_equipo = request.form.get('fecha_prestamo_equipo')
        fecha_entrega_equipo = request.form.get('fecha_entrega_equipo')
        ...
        # 1. Si el estado anterior era PRÉSTAMO y el nuevo es diferente
        ...
        # 3. Si el nuevo estado es PRÉSTAMO, crear un nuevo registro
       
        if nuevo_estado == 'PRÉSTAMO':
            cur.execute("""
                INSERT INTO prestamos_equiposalud (cod_articulo, nombre_equipo, quien_entrega_equipo, quien_recibe_equipo, ubicacion_original, ubicacion_destino_laboratorio, fecha_prestamo_equipo, fecha_entrega_equipo, enable_prestamos)
        ...
        return redirect(url_for('indexSalud'))
# =======================================//======================================================= OJO REVISAR PARA MOSTRAR LOS DATOS ESTADO DE PRÉSTAMO 
# FUNCIÓN PARA MOSTRAR LOS DATOS DE LOS EQUIPOS QUE SE ENCUENTRAN PRESTADOS
@app.route('/prestamos_equipos_salud/<cod_articulo>')
